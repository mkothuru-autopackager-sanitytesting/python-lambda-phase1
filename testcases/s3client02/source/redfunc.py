...
def lambda_handler(e, c):
    ...
    c = client = session.create_client('s3', region_name='us-west-2')
    ...
    if "x" in e and "y" in e:
        x = e["x"]   # SOURCE
        r = c.client.copy(CopySource=x, Bucket=x, Key=x)   # CWEID 99
    if "x" in e and "y" in e:
        x = e["x"]   # SOURCE
        y = e["y"]   # SOURCE
    ...
    if "x" in e and "y" in e:
        x = e["x"]   # SOURCE
        r = c.upload_file(Filename="foo", Bucket=x, Key=x)   # CWEID 99
    if "x" in e and "y" in e:
        x = e["x"]   # SOURCE
        r = c.upload_fileobj(Fileobj=open("foo", "r"), Bucket=x, Key=x)   # CWEID 99
    if "x" in e and "y" in e:
        x = e["x"]   # SOURCE
    ...
    if "x" in e and "y" in e:
        x = good_bucket
        r = c.client.copy(CopySource=x, Bucket=x, Key=x)
    if "x" in e and "y" in e:
        x = good_bucket
        y= "Ok" 
    ...
    if "x" in e and "y" in e:
        x = e["x"]   # SOURCE
        r = c.create_multipart_upload(Bucket=good_bucket, Key=good_key, ACL=x)   # CWEID 284
...
# add wildcard
    ...
    r = c.put_bucket_inventory_configuration(Bucket=good_bucket, InventoryConfiguration=ZZZ)   # CWEID 547, 99, 532


    se = os.environ["AWS_SECRET_ACCESS_KEY"]   # SOURCE SENSITIVE
    c.client.copy(Bucket=good_bucket, Key=se)   # CWEID 201
    ...
    se = os.environ["AWS_SECRET_ACCESS_KEY"]   # SOURCE SENSITIVE
    c.create_multipart_upload(Bucket=good_bucket, Key=good_key, SSECustomerKey=HARDCODED, CopySourceSSECustomerKey=HARDCODED)   # CWEID 201
    se = os.environ["AWS_SECRET_ACCESS_KEY"]   # SOURCE SENSITIVE
    ...
    c.put_bucket_website( WebsiteConfiguration=se)   # CWEID 201
